...
out_file <- here(params$obj_dir, str_c(sam_grp, "_exon_pausing.tsv.gz"))
...
get_ss_grps <- function(df_in, sam, genes = NULL) {
  res <- df_in %>%
    ...
    select(name, type, win_id, counts) %>%
    ...
    select(name, type, gene_name, grp, seq)
...
out <- here(params$obj_dir, str_c(sam_grp, "_ss.rds"))
