...
# File for saving plot data
tss_body_wins_file <- file.path(plot_params$obj_dir, str_c(grp, pause_prfx, "tss-body-wins_data.tsv.gz"))
tss_body_file      <- file.path(plot_params$obj_dir, str_c(grp, pause_prfx, "tss-body_data.tsv.gz"))
```
...
For the gene body region (`r body_desc`), reads were downsampled so the body
...
* data used for this plot is saved in `r tss_body_wins_file`
...
# Save data used for boxplots
box_dat_0 %>%
  write_tsv(here(tss_body_wins_file))
...
The fraction of reads aligning to pauses is shown for TSS and gene body regions. p-values were calculated using a Wilcoxon rank sum test.
...
* data used for this plot is saved in `r tss_body_file`
...
* TSS regions >=`r TSS_LIM` reads
* body region >=`r BODY_LIM` read/kb
* all regions >= `r PAUSE_LIM[2]` pause (this is to exclude zeros)
...
```{r "{{grp}} exon intron header", results = "asis", eval = do_splicing}

cat("\n\n### Exon intron pausing\n\n")
cat("The fraction pause reads is shown below for exons and introns.\n")
cat("For each gene the total number of reads aligning to pause sites were summed for all exons or introns in the gene.\n")
cat("The total pause reads were then divided by the total number of exonic or intronic reads for the gene.\n\n")
cat(
  "* genes >", MIN_ZONE_GENE_LEN,
  " kb long and separated by >", extract_sep_info(plot_params$genes_zone),
  " kb are shown (", basename(plot_params$genes_zone), ")\n",
  sep = ""
)
cat("* exon/intron annotations were used for the transcript with the start site matching the start site used for gene lists\n")
cat("* single exon genes were excluded\n")
cat("* for transcripts with multiple isoforms that have the same start site, the highest confidence isoform was used\n")
cat("* if multiple isoforms have the same confidence level, the gene was excluded (this is generally a small number ~400)\n")
cat("* entire gene region downsampled\n")
cat("* there must be at least 1 total pause in both exonic and intronic regions\n\n")

```

```{r "{{grp}} LOAD EXON INTRON DATA", eval = do_splicing}
...
```{r "{{grp}} SUM EXON INTRON COUNTS", eval = do_splicing}
...
```{r "{{grp}} EXON INTRON PLOTS 1", fig.width = 12, fig.height = 1 + (2.5 * n_reps), eval = do_splicing}
...
```{r "{{grp}} EXON INTRON PLOTS 2", fig.width = 12, fig.height = 6.5, eval = do_splicing}
...
The fold change in pause strength vs pause density is shown below for the TSS (`r tss_desc`) and gene body (`r body_desc`) regions.
