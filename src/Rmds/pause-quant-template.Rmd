...
create_stat_corr <- function(df_in, x, y, ttl = NULL, plt_clrs = c("black", last(clrs))) {
  ...
  corr_dat <- corr_dat %>%
    ...
    mutate(
      n     = n_distinct(name),
      n     = str_c("n = ", comma(n)),
      corr  = list(broom::tidy(cor.test(log10(!!sym(x)), !!sym(y)))),
...
ex_int_beds <- ex_int_beds %>%
  dplyr::filter(transcript %in% ex_int_txns) %>%
...
plt_dat <- pause_stats %>%
  ...
  dplyr::filter(region %in% NEW_regs) %>%
  ...
  dplyr::select(-len, -sample, -sub_reg) %>%
  pivot_longer(all_of(metrics), names_to = "type", values_to = "counts") %>%

  # remove genes that do not have data for all regions
  pivot_wider(names_from = "region", values_from = "counts") %>%
  dplyr::filter(if_all(all_of(NEW_regs), ~ !is.na(.x))) %>%
...
plt_dat <- pause_stats %>%
  dplyr::filter(sub_reg %in% plt_regs) %>%
  dplyr::select(-sample, -sub_reg, -len) %>%
  ...
  # remove genes that do not have data for all regions
  pivot_wider(names_from = "region", values_from = "counts") %>%
  dplyr::filter(if_all(all_of(plt_regs), ~ !is.na(.x))) %>%
...
x %>%
  dplyr::filter(nonzero != "none") %>%
  
  pivot_wider(names_from = "treat", values_from = "counts") %>%
  group_by(rep, nonzero) %>%
  mutate(
    p = broom::tidy(wilcox.test(WT, TE7))$p.value,
    p = scientific(p),
    n = str_c(n, "\np = ", p)
  ) %>%
  ungroup() %>%
