...
create_cor_plots <- function(df_in, x, y, clr = "red", pt_size = 1, out_size = 1.5) {
  ...
  res <- df_in %>%
    mutate(
      corr = cor(log10(!!sym(x)), log10(!!sym(y))),
      corr = str_c(n, "\nr = ", round(corr, 2))
    ) %>%
...
Pausing zones were identified as the window (200 bp) where the fraction of pause reads is >`r cutoff` and the average for the prospective zone is >`r cutoff * ave_mtplyr`. Metaplots (200 bp windows) are shown for different pausing metrics. The vertical lines show the mean length of the pausing zone for each sample.
...
```{r "{{grp}} zone metaplots", fig.width = 10, fig.height = 1 + (n_reps * 2.5)}
...
sm_zone_wins <- sfxs %>%
  imap_dfr(~ {
    typ <- names(.x)
    out <- here(plot_params$obj_dir, str_c(grp, "_pause_zones_", typ, "_5_10bp.tsv.gz"))
...
# Data for pause zones
# Some windows are slightly longer than the gene length since the win size
