---
title: <p style="font-size:45px;">CCl4 eNET-seq</p>
...
exons <- read_tsv(str_c(list_dir, plot_params$exons))
...
tx_info <- here(list_dir, plot_params$trxn_info) %>%
  ...
  dplyr::select(transcript, support)
...
exon_lens <- exons %>%
  ...
  left_join(tx_info, by = "transcript") %>%
  
  dplyr::select(
    chrom, start, end, score,
    strand, gene_name, transcript, ex_coords,
...
high_conf_txns <- unique(exon_lens$transcript)
...
ex_int_txns <- exon_lens %>%
  group_by(gene_name) %>%
  dplyr::filter(n_distinct(transcript) == 1) %>%
  pull(transcript)
