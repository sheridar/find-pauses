...
merge_5_nosub <- load_merge_wins(
  ...
  win_num   = meta_geo$tss$win_num,   # Total number of expected windows (including sense + anti)
  ref_win   = meta_geo$tss$ref_win,   # Reference window for adjusting window ids
  win_min   = meta_geo$tss$win_min    # Min window for filtering
) %>%
  check_group_size(
...
merge_3_nosub <- load_merge_wins(
  ...
  overwrite = plot_params$overwrite,  # Overwrite output file if it exists
  col_names = win_cols,               # Column names for bed files
  win_num   = meta_geo$tes$win_num,   # Total number of expected windows (including sense + anti)
  ref_win   = meta_geo$tes$ref_win,   # Reference window for adjusting window ids
  win_min   = meta_geo$tes$win_min,   # Min window for filtering
  win_max   = meta_geo$tes$win_max    # Max window for filtering
) %>%
  check_group_size(
    clmns = c("name", "sample", "type", "win_id"),
    quiet = TRUE
...
mean_meta_fig <- create_meta_fig(
  ...
  brk_5     = meta_geo$tss$brk,
  lab_5     = meta_geo$tss$lab,
  brk_3     = meta_geo$tes$brk,
  lab_3     = meta_geo$tes$lab
)
...
z5_plts <- z_dfs %>%
  imap(~ {
    res <- .x %>%
      dplyr::filter(win_dist >= -meta_geo$tss$zoom_lim & win_dist <= meta_geo$tss$zoom_lim) %>%
      create_meta(
        x         = "win_dist",
        y         = "counts",
        color     = "treat",
      ...
      labs(y = .y) +
      add_breaks(meta_geo$tss$zoom_brk, meta_geo$tss$lab) +
      meta_theme
...
z3_plts <- z_dfs %>%
  imap(~ {
    res <- .x %>%
      ...
      labs(y = .y) +
      add_breaks(meta_geo$tes$brk, meta_geo$tes$lab) +
      meta_theme
...
```{r "{{.x}} ss header", results = "asis", eval = do_splicing}

cat("\n\n#### \n\n")
cat("Mean NET-seq signal is shown below for the region around 5' splice sites.\n\n")
cat(
  "* genes >", MIN_GENE_LEN,
  " kb long and separated by >", extract_sep_info(plot_params$genes_5),
  " kb are shown\n\n",
  sep = ""
)
...
```{r "{{.x}} ss functions", eval = do_splicing && n_treats > 1}
...
```{r "{{.x}} meta exons NO SUBSMAPLING", eval = do_splicing && n_treats > 1}
...
```{r "{{.x}} ss data NO SUBSMAPLING", eval = do_splicing && n_treats > 1}
...
```{r "{{.x}} ss metaplot NO SUBSMAPLING", fig.width = 10, fig.height = 1 + (2 * n_reps), eval = do_splicing && n_treats > 1}
...
`r if (do_zone) knitr::knit_child(text = c("## Pausing zones", "", chunks$zone))`
