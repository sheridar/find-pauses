...
fq_sfx <- "(_S[0-9]+|)(_L[0-9]+|)(_R[12]_[0-9]+.fastq|_[12].fq).gz$"
...
fq_sum <- fq_sum %>%
  ...
  trim_path(
    rm_str   = c("_S[0-9]+_L[0-9]{3}", "_[0-9]{3}.fastq.gz$", ".fq.gz$", "^NF_"),
    file_col = "path"
  ) %>%
...
fq_sum %>%
  ...
  scale_x_discrete(labels = ~ str_trunc(.x, side = "right", width = 30)) +
  theme_info +
  theme(
    aspect.ratio    = ratio,
    strip.clip      = "off",
...
if (!identical(sort(names(qc_sams)), sort(unique(sub_sum$grp)))) {
  stop("groups specified in SAMPLES.yaml do not match those in PLOTS.yaml, check your PLOTS.yaml config")
}
